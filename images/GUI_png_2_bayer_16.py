...
def submit():
    ...
    call = f"python png2bayerCFA_GB_16.py -i {folder_in}/original -o {folder_out}/bayerCFA_GB -l {lossy_bits} -f {filename} -b {bpp_entry.get()}"
...
# Title
root.title("PNG16 to Bayer CFA GB Binary Converter")
title = tk.Label(root, text="PNG16 to Bayer CFA GB Binary Converter", font=("Helvetica", 20))
title.pack()
...
# BPP (8,10,12,14 or 16)
bpp_label = tk.Label(root, text="BPP:(8,10,12,14 or 16)")
bpp_label.pack()
bpp_entry = tk.Entry(root, width=10)
bpp_entry.insert(0, "16")
bpp_entry.pack()
