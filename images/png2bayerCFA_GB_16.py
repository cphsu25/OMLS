...
def dump_help():
    ...
    print("   -b (--bpp): [Bits per pixel. (default 16).]")
...
def main(argv):
    ...
    # for imgIdx in range(minIdx, maxIdx + 1):
            ...
            # read image data
            img2d = np.empty((height, width * planes), dtype=np.uint16)
            for i, el in enumerate(pngdata):
                img2d[i, :] = list(map(np.uint16, el))
            ...
            if border != 0:
                ...
                w = png.Writer(width=width, height=height, greyscale=False, bitdepth=16)
                f = open(f"{output_loc}/bayerCFA_GB/cut_{filenameFormatOut}{imgIdx:02}.png", "wb")
                imgForPng = img2d.astype(np.uint16).flatten()
            ...
            if alreadyBayer:
                bayerCFA = img2d[:, :, 0].astype(np.uint16)
            else:
            ...
            img2d = np.empty((height, width), dtype=np.uint16)
            for i, el in enumerate(pngdata):
                img2d[i, :] = list(map(np.uint16, el))
